...
Here's another forex quote that helps 
make clear the meaning  of these terms in the forex market:
    EUR/USD = 1.3600/05
Here the bid is 1.3600, and the ask is 1.3605. 
Since the difference between a bid and an ask price 
in normal circumstances is a very small fraction
 -- less than 1/100th of the currency unit -- the
  convention is that only the last two digits (05)
...
Margin is calculated based on the current market quote of the base currency of the trader’s account
 vs base currency of the trader’s account, the volume requested, and the leverage level of the trader’s account.
...
It is important to note that although the above mechanism may protect you from negative equity, it is not a guaranteed protection. 
In the event of certain pricing actions beyond our control (for example market volatility or the market closing and then opening again at a different rate,
 such as on a weekend)  your positions may not be automatically closed at the margin levels set out above. 
...
body:
`Multi Account Manager account types on the Meta-Trader 4 platform are designed for Money Managers who trade on behalf of other investors 
and manage multiple accounts from a single interface. Money Managers can also manage multiple accounts by utilising Expert Advisors (EAs).
`
},
{title:"Safe Haven Currencies",
body:
`This is a term used to describe trading an alternative currency or instrument that is less volatile as a result of market turmoil and uncertainty.
Safe haven currencies or instruments are considered low risk because their issuing governments are stable and their economies tend to be strong, 
however, this does not necessarily mean that they are ‘safe’.`
},

{title:"Trailing Stop",
body:
`A "Trailing Stop" applies a special algorithm to a Stop Loss order.
When a Trailing Stop is placed, the level of the Stop Loss will change along with price movement. 
If you open a Buy position, and the price of the instrument increases by a certain number of points,
...
Generally speaking, a Trailing Stop causes the level of the Stop Loss to trail the price level.

 Source: "What is a trailing stop? Will one be triggered if I am offline?"
`
}
]
        },
